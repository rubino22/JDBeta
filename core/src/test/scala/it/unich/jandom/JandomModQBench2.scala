 ...
 * based ones.
 */
object JandomModQBench2 extends App {
  ...
  var totalPrograms = 0
  var totalTimeReduce =0.0
  var totalTimePtope =0.0
...
*/    
    val params2 = new targets.Parameters[LTS] { val domain = ParallelotopeDomainModQSpire(0,overRound=false) }
...
/*
  ...
  */
    ...
    // val params5 = new targets.Parameters[LTS] { val domain = ParallelotopeDomainModQSpire(0,overRound=false) }
...
*/
  ...
  //   val cpar = par mapValues { p => CStoPolyehdra(p.dimension, p.constraints) }
   totalTimeReduce=totalTimeReduce + tann2;
   totalTimePtope=totalTimePtope + tann5 ;
...
//if(comp.count(_._2 == Some(1))>0) {Q += s"------>${model}"}
    ...
    println(s"Times Altro Vs ProdDOm:  ${tann5} vs  ${tann2}")
    //println("WIDENINGS: " + program.locations.filter(program.isJoinNode).map(_.name).mkString(", "))
...
*/
  ...
  print(s"Total time Reduce: ${totalTimeReduce}  ")
  print(s"  vs    Total time Ptope: ${totalTimePtope}")
