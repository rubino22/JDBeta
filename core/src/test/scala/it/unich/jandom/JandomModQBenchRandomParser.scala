 ...
 * based ones.
 */
object JandomModQBenchRandomParser extends App {
  ...
  def fastModelAnalyze(model: File) = {
   ...
   // val source = scala.io.Source.fromFile(model.getAbsolutePath).mkString
    ...
    // parsed.parseProgram(source.toString()) match {    
  ...
  /*
    ...
    val params2 = new targets.Parameters[SLILTarget] { val domain = PPLDomain[C_Polyhedron]()}
  ...
  params2.narrowingFactory = DelayedNarrowingFactory(DefaultNarrowing, 3)
   // params2.debugWriter = new java.io.PrintWriter(System.out)
    ...
    val params3 = new targets.Parameters[SLILTarget] { val domain = ParallelotopeDomain() }
    ...
    val params4 = new targets.Parameters[SLILTarget] { val domain = ParallelotopeDomainModRationalGmpExt(overRound=false) }
    ...
    */
    ...
    val params5 = new targets.Parameters[SLILTarget] { val domain = BoxDoubleDomain() }
   ...
   /* val params6 = new targets.Parameters[SLILTarget] { val domain = ParallelotopeDomainModQSpire(overRound=false) }
   ...
   /* print("PPL Octagon_Shape_double: ")
    ...
    //comparing sum with parallelotope
   /* val comp = cann2 map { case (loc, v) => (loc -> v.tryCompareTo(cann6(loc))) }
    ...
    /*case parser.NoSuccess(msg, next) =>
           println( "Error in parsing source code")
        None
    }*/
  }

  val resources = getClass.getResource("/random/").toURI;
  //val resources2 = getClass.getResource("/random/octagon-6.R").toURI;
