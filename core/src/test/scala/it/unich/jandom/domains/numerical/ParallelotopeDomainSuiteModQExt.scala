 ...
 */
class ParallelotopeDomainSuiteModQExt extends NumericalDomainSuite with SeparatedTopAndBottomSuite with EmptyExistsSuite {
  lazy val dom = ParallelotopeDomainModRationalGmpExt()
  ...
  describe("rotation of shapes") {
    ...
    val m1 = DenseMatrix.zeros[ModRationalGmpExt](m.rows, m.cols)
    for(i <- 0 until m.rows){for (j <- 0 until m.cols){m1(i,j)=ModRationalGmpExt(m(i,j))}}
