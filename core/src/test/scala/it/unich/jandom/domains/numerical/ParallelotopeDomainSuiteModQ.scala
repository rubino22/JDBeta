 ...
 */
class ParallelotopeDomainSuiteModQ extends NumericalDomainSuite with SeparatedTopAndBottomSuite with EmptyExistsSuite {
  lazy val dom = ParallelotopeDomainModQ()
  ...
  describe("rotation of shapes") {
    ...
    val m1 = DenseMatrix.zeros[ModQ](m.rows, m.cols)
    for(i <- 0 until m.rows){for (j <- 0 until m.cols){m1(i,j)=ModQ(m(i,j))}}
