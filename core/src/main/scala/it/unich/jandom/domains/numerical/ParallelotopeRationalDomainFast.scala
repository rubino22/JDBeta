...
import it.unich.jandom.utils.numberext.DenseVector
    ...
    //  require(dimension == that.dimension)
      if (isEmpty)
       ...
       if(this >= that){
		...
		def unionWidening(that: Property): Property = {
				...
				type PrioritizedConstraint = (DenseVector, RationalExt, RationalExt, Int)
						 ...
						 * that (2). This is used to refine priorities.
						 */
						def priority(v: DenseVector, ownedBy: Int = 2): PrioritizedConstraint = {
					val y1 = A.t \ v
							val (l1, u1) = domain.extremalsInBox(y1, low, high)
							val y2 = that.A.t \ v
							val (l2, u2) = domain.extremalsInBox(y2, that.low, that.high)
          ...
          if (!l1.isInfinity && !l2.isInfinity && !u1.isInfinity && !u2.isInfinity) {
            if (l1 == l2 && u1 == u2)
              10
            else if (l1 >= l2 && u1 <= u2)
              if (ownedBy == 2) 20 else 30
								...
								if (isEmpty)
									"empty"
										...
										val eqns = for (i <- 0 until dimension) yield {
											if (low(i) < high(i))
												s"${if (low(i).isNegInfinity) "-∞" else low(i)} ≤ ${lfToString(A.row(i))} ≤ ${if (high(i).isPosInfinity) "+∞" else high(i)}" //=>col(i)=>row(i)
												else
													s"${lfToString(A.row(i))} = ${high(i)}" //col(i) =>row(i)
										}
										eqns.mkString("[ ", " , ", " ]")
									}
						}
