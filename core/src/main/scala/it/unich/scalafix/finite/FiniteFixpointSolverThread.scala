...
import it.unich.scalafix.finite.FiniteFixpointSolverThread.Params
...
/*
    class MyThread[U, V: Domain, E](Gp:GraphEquationSystem[U, V, E], CC77: Params[U, V], nThread:Int,current:mutable.Map[U,V],lister: FixpointSolverListener[U, V] ) extends Callable[Assignment[U, V]] {
//class MyThread[U, V: Domain, E](Gp:GraphEquationSystem[U, V, E], CC77: Params[U, V], nThread:Int,current:mutable.Map[U,V] ) extends Callable[Assignment[U, V]] {
import FiniteFixpointSolverThread._
 override def call:Assignment[U, V] = {
  val threadId = Thread.currentThread().getId();
 // println("MyID"+threadId);
   //val result = FiniteFixpointSolverThread(Gp, CC77, nThread,current,lister)

  val result = FiniteFixpointSolverThread(Gp, CC77, nThread,current)
   return result
  }
}

   class VariabileCondivisa[U, V: Domain, E](Gp:GraphEquationSystem[U, V, E]) {
    val start: Assignment[U, V] =Gp.initial
 @volatile   var current = mutable.HashMap.empty[U, V].withDefault(start) 
 var lister: FixpointSolverListener[U, V] = EmptyListener
 
  }
*/

object FiniteFixpointSolverThread  {
  ...
  private def applySolver[U, V] (
                                 ...
                                 lista:List[Iterable[U]]
                                 ): Assignment[U, V] = {
    solver match {
      case Solver.RoundRobinSolver => RoundRobinSolver(eqs)(start, listener)
      case Solver.RoundRobinSolverThread => RoundRobinSolverThread(eqs,lista)(start, listener)
      case Solver.KleeneSolver => KleeneSolver(eqs)(start, listener)
      case Solver.KleeneSolverThread => KleeneSolverThread(eqs,lista)(start, listener)
      case Solver.WorkListSolver => WorkListSolver(eqs)(start, listener)
      case Solver.WorkListSolverThread => WorkListSolverThread(eqs,lista)(start, listener)
  ...
  def CC77[U, V](solver: Solver.Solver, wideningBox: Box[V], narrowingBox: Box[V]) =
    ...
    */
  def apply[U, V: Domain, E](eqs: GraphEquationSystem[U, V, E], params: Params[U, V], lista: List[Iterable[U]]): Assignment[U, V] =  {
    import params._
    ...
    boxstrategy match {
      case BoxStrategy.OnlyWidening =>
        ...
        applySolver(solver, withWidening, startAssn, ordering, restart, listener, lista)
       // applySolver(solver, withWidening, startAssn, ordering, restart, listener, nthread,current)
      case BoxStrategy.TwoPhases =>
        val widening = boxFilter[U, V](eqs, wideningBox, boxlocation, ordering)
        val withWidening = boxApply(eqs, widening, boxscope, ordering)
        listener.ascendingBegins(startAssn)
        val ascendingAssignment = applySolver(solver, withWidening, startAssn, ordering, restart, listener,lista)
      //    val ascendingAssignment = applySolver(solver, withWidening, startAssn, ordering, restart, listener,nthread,current)
        ...
        applySolver(solver, withNarrowing, ascendingAssignment, ordering, restart, listener,lista)
       //  applySolver(solver, withNarrowing, ascendingAssignment, ordering, restart, listener, nthread,current)
      case BoxStrategy.Warrowing =>
        if (boxscope == BoxScope.Localized) {
          ...
          applySolver(solver, withUpdate, startAssn, ordering, restart, listener,lista)
         //    applySolver(solver, withUpdate, startAssn, ordering, restart, listener,nthread,current)
        } else {
          ...
          applySolver(solver, eqsWithWarrowing, startAssn, ordering, restart, listener, lista)
         //   applySolver(solver, eqsWithWarrowing, startAssn, ordering, restart, listener, nthread,current)
        }
    }
  }
