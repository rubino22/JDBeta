...
import FiniteFixpointSolver._
 override def call:Assignment[U, V] = {
  val threadId = Thread.currentThread().getId();
 // println("MyID"+threadId);
   val result = FiniteFixpointSolver(Gp, CC77,nThread,current,lister)
   return result
  }
}
...
object FiniteFixpointSolver  {
  ...
  private def applySolver[U, V] (
       ...
       // println(eqs+"   "+start+"  "+listener)
    solver match {
  
      case Solver.RoundRobinSolver => RoundRobinSolver(eqs)(start, listener)
      //case Solver.RoundRobinSolverThread2 => RoundRobinSolverThread2(eqs)(start, listener)
  ...
  def apply[U, V: Domain, E](eqs: GraphEquationSystem[U, V, E], params: Params[U, V]): Assignment[U, V] =  {
    ...
    boxstrategy match {
      ...
      //  println(solver+" | "+withNarrowing+" | "+ascendingAssignment+" | "+ordering+" | "+restart+" "+listener)
